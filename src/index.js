...
function start(client) {
  client.onMessage( async (message) => {
    ...
    if(msg.indexOf("!on") != -1){
      ...
      console.log("statusM");
      client
        .sendText(message.from, `[${message.sender.pushname}] está: ${banco[message.author].status}`)
        .then((result) => {
      ...
      for(let i = 0; i < members.length; i++){
        if(banco[members[i]._serialized].name != undefined){
          ...
          client.sendText(message.from, `[${banco[position].name}] está: ${banco[position].status}\n\n`);
        }
      }
      ...
      for(let i = 0; i < members.length; i++){
        ...
        if(banco[members[i]._serialized].name != undefined && banco[members[i]._serialized].all != false){
          client.sendMentioned(message.from, `@${members[i].user}`, [members[i].user]);
        }
      }
    }
  });
